...
import { DemonstratorsGallery } from './DemonstratorImages';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-stretch">
          ...
          <div className="lg:col-span-6 space-y-6 flex flex-col justify-between">
            ...
            {/* Target Demonstrators / Key Technology Applications (3 images side by side, last one UV.jpg) */}
            <DemonstratorsGallery />
          </div>

          {/* Right Column: Subprojects Listing (SP code, PID reference, Coordinator tag, Funding, Logo, and Title only) */}
          <div className="lg:col-span-6 flex flex-col">
            <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4 flex flex-col justify-between flex-1 h-full">
              ...
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-3.5">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-mono mt-0.5 leading-snug">
                    Coordinated Research Subprojects<br />
                    & Funding Breakdown (PID-2025)
                  </p>
                </div>

                <button
                  onClick={() => onSelectTab ? onSelectTab('consortium') : undefined}
                  className="inline-flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-800 border border-slate-200 px-3.5 py-2 rounded-lg font-semibold text-xs transition-colors shadow-2xs cursor-pointer shrink-0"
                >
              ...
              <div className="flex-1 flex flex-col justify-between gap-3 py-1">
                {subprojects.map((sp) => (
                  <div
                    ...
                    className="p-3.5 sm:p-4 rounded-xl border border-slate-200 hover:border-slate-300 hover:bg-slate-50/50 transition-all hover:shadow-xs bg-white cursor-pointer flex flex-col justify-center flex-1"
                  >
                    {/* Top Row: Subproject Code, Reference, Funding, and Institutional Logo (Identical across all 4 subprojects) */}
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-2.5 mb-2.5">
                      <div className="flex items-center gap-2">
                        <span
                          className="font-mono text-xs font-bold px-2.5 py-0.5 rounded text-white shrink-0"
                      ...
                      {/* In the case of UPC (SP1), the Coordinator badge is on the line below (under SP1) */}
                      {sp.id === 'sp1' && (
                        <div className="w-full pt-1 flex items-center">
                          <span className="text-[10px] font-mono font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded inline-block">
                            COORDINATOR
                          </span>
                        </div>
                      )}
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2 text-xs font-mono mt-auto">
